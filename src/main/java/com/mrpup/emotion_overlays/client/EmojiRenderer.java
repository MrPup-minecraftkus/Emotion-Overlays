...
public class EmojiRenderer {
    ...
    public static void onSubmitCustomGeometry(SubmitCustomGeometryEvent event) {
        ...
        for (Player player : mc.level.players()) {
            ...
            event.getSubmitNodeCollector().submitCustomGeometry(ps, renderType, (pose, consumer) -> {
    ...
    private static void drawQuad(PoseStack.Pose pose, VertexConsumer consumer, int argb) {
        ...
        consumer.addVertex(pose, -h, -h, 0).setColor(r, g, b, a).setUv(0, 0).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(pose, 0, 0, 1);
        consumer.addVertex(pose,  h, -h, 0).setColor(r, g, b, a).setUv(1, 0).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(pose, 0, 0, 1);
        consumer.addVertex(pose,  h,  h, 0).setColor(r, g, b, a).setUv(1, 1).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(pose, 0, 0, 1);
        consumer.addVertex(pose, -h,  h, 0).setColor(r, g, b, a).setUv(0, 1).setLight(0xF000F0).setOverlay(OverlayTexture.NO_OVERLAY).setNormal(pose, 0, 0, 1);
    }
