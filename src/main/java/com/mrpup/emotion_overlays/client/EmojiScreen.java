...
public class EmojiScreen extends Screen {
    ...
    private void renderTabs(GuiGraphicsExtractor g, int mx, int my) {
        ...
        g.centeredText(font, "<", panelX + arrowW / 2, panelY + (TAB_H - 8) / 2, tabOffset > 0 ? 0xFFFFFFFF : 0xFF555555);
        ...
        for (int i = 0; i < VISIBLE_TABS; i++) {
            ...
            g.centeredText(font, label, tx + tabW / 2, ty + (TAB_H - 8) / 2, active ? 0xFFFFFFFF : 0xFF888888);
        }
        ...
        g.centeredText(font, ">", rightArrowX + arrowW / 2, panelY + (TAB_H - 8) / 2,
                (tabOffset + VISIBLE_TABS < catNames.size()) ? 0xFFFFFFFF : 0xFF555555);
    }

    private void renderSearch(GuiGraphicsExtractor g, int mx, int my) {
        int sx = panelX + PAD;
        ...
        g.text(font, display, sx + 4, sy + (sh - 8) / 2, 0xFFCCCCCC, false);
    }

    private void renderGrid(GuiGraphicsExtractor g, int mx, int my) {
        hoveredIndex = -1;
        ...
        for (int vi = 0; vi < visible; vi++) {
            ...
            Identifier tex = EmojiTextureManager.getTexture(entry);
            if (tex != null) {
                g.blit(tex,
                        cx + offset, cy + offset,
                        cx + offset + iconSize, cy + offset + iconSize,
                        0f, 1f, 0f, 1f);
            } else {
                g.centeredText(font, entry.character(), cx + CELL / 2, cy + (CELL - 8) / 2, 0xFFAAAAAA);
        ...
        if (hoveredIndex >= 0) {
            ...
            g.setTooltipForNextFrame(font, Component.literal(e.character() + " " + e.name()), mx, my);
        }
    }

    private void renderScrollBar(GuiGraphicsExtractor g, int mx, int my) {
    ...
    private void renderStatus(GuiGraphicsExtractor g) {
        ...
        g.centeredText(font, msg, panelX + panelW / 2, sy + 2, col);
    }
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        int imx = (int) event.x(), imy = (int) event.y();
        int arrowW = 20;
        ...
        if (hoveredIndex >= 0 && hoveredIndex < currentList.size()) {
            ...
            ClientPacketDistributor.sendToServer(new SelectEmojiPacket(selected.cpHex()));
            onClose();
            return true;
        }
    ...
    public boolean mouseDragged(MouseButtonEvent event, double dx, double dy) {
        if (draggingScroll && totalRows > GRID_ROWS) {
            double frac = (event.y() - scrollBarY0) / (double) scrollBarH;
